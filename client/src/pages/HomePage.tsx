          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className="inline-flex items-center gap-1 text-[10px] font-black text-emerald-400 uppercase tracking-widest bg-emerald-950/60 border border-emerald-900 px-2.5 py-1 rounded-md">
                  <ShieldCheck size={11} />
                  Secured Ledger
                </span>
                <span className="inline-flex items-center gap-1 text-[10px] font-black text-indigo-400 uppercase tracking-widest bg-indigo-950/60 border border-indigo-900 px-2.5 py-1 rounded-md">
                  <Cpu size={11} />
                  Analyst Panel
                </span>
              </div>
              ...
              <p className="text-slate-400 text-xs mt-1.5 max-w-lg leading-relaxed font-semibold">
            ...
            <div className="shrink-0">
              <Link
                to={PAGES.NEW_LOG}
                className="inline-flex items-center gap-2 px-4.5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl transition-all shadow-lg shadow-indigo-650/30 cursor-pointer uppercase tracking-wider active:scale-[0.98]"
              >
                <Plus size={14} />
                New entry
              </Link>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden group">
            ...
            <span className="text-xl font-black text-slate-950 block">
              {isLoading ? '—' : totalCane.toLocaleString()} <span className="text-xs text-slate-500 font-bold uppercase">Qtls</span>
            </span>
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden group">
            ...
            <span className="text-xl font-black text-indigo-650 block">
              {isLoading ? '—' : totalSugar.toLocaleString()} <span className="text-xs text-slate-500 font-bold uppercase">Qtls</span>
            </span>
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden group">
            ...
            <span className="text-xl font-black text-slate-850 block">
              {isLoading ? '—' : logs.length} <span className="text-xs text-slate-500 font-bold uppercase">Logs</span>
            </span>
        ...
        <div className="grid lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden lg:col-span-2 flex flex-col">
            ...
            {isLoading ? (
              ...
              <div className="p-8 text-center text-xs text-slate-550 font-bold uppercase tracking-wider">
                No logs recorded yet. Click "+ New Entry" to begin!
              </div>
            ) : (
          ...
          {/* Sidebar Panel: Factory Quick Information */}
